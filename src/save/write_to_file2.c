...
void save_player_type(FILE *fd)
{
    ...
    fwrite(my_itoa(all_infos()->player_type), 1,
    my_strlen(my_itoa(all_infos()->player_type)), fd);
    fwrite("\n", 1, 1, fd);
}
...
void save_nb_enemies(FILE *fd)
{
    ...
    fwrite(my_itoa(all_infos()->nb_of_enemies_outside), 1,
    my_strlen(my_itoa(all_infos()->nb_of_enemies_outside)), fd);
    fwrite("\n", 1, 1, fd);
}
